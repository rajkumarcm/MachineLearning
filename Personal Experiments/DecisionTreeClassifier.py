...
class DecisionTreeClassifier:
    ...
    def gini_impurity(self, y):
        ...
        for target_val in np.unique(y):
            p = np.sum(y == target_val) / n
            gini -= p ** 2
        return float(gini)
    
    def gini_impurity_split(self, left, right):
        n_left = left.shape[0]
    ...
    def find_best_split(self, df, column, y):
        ...
        if np.issubdtype(df[column].dtype, np.number):
            ...
            for thresh in candidate_thresholds:
                ...
                gini_vals[thresh] = self.gini_impurity_split(left, right)
            ...
            for thresh in candidate_thresholds:
                ...
                gini_vals[thresh_str] = self.gini_impurity_split(left, right)
    ...
    def find_best_attribute(self, df, y):
        ...
        att_threshold = {}
        for cname in cnames:
            optimal_thresh, gini_val = self.find_best_split(df, cname, y)
            att_gini[cname] = gini_val
            att_threshold[cname] = optimal_thresh
        best_attribute = min(att_gini, key=att_gini.get)
        best_threshold = att_threshold[best_attribute]
        gini_val = att_gini[best_attribute]
        return best_attribute, best_threshold, gini_val
    ...
    def build_tree(self, df, y, depth=0):
        ...
        majority_class = y_vals[np.argmax(y_counts)]
        ...
        best_att, best_thresh, best_att_gini = self.find_best_attribute(df_sub, y)

        # 4. If no attribute can be picked as the best, then default to the leaf node
        info_gain = parent_gini - best_att_gini
